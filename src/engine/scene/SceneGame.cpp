...
void engine::SceneGame::activateVisibility()
{
	...
	mapSize.x += 2;
	mapSize.y += 2;
	Irrlicht::getInstance()->getSceneManager()->addCameraSceneNode(nullptr, \
	irr::core::vector3df(mapSize.x / 2, std::max(mapSize.x, mapSize.y) * 0.8, mapSize.y * 0.7), irr::core::vector3df(mapSize.x / 2, -3, mapSize.y / 2));
	  ...
	  _p4_dead(false)
{
	...
	terrain->setMaterialTexture(0,
		driver->getTexture("res/sprite/img/terrain-texture.jpg"));
	terrain->setMaterialTexture(1,
		driver->getTexture("res/sprite/img/detailmap3.jpg"));
